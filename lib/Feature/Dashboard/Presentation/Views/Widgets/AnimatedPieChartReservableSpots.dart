...
class AnimatedPieChartReservableSpots extends StatefulWidget {
  static String id = 'AnimatedPieChart';
  final List<GetPopularReservableSpotModel> reservablespot;

  const AnimatedPieChartReservableSpots(
      {super.key, required this.reservablespot});
  @override
  _AnimatedPieChartReservableSpotsState createState() =>
      _AnimatedPieChartReservableSpotsState();
}

class _AnimatedPieChartReservableSpotsState
    extends State<AnimatedPieChartReservableSpots> {
  int touchedIndex = -1;
        ...
        child: Stack(
          ...
          children: [
            AspectRatio(
              ...
              child: PieChart(
                PieChartData(
                  sections: _buildSections(widget.reservablespot),
                    ...
                    touchCallback: (event, response) {
                      setState(() {
                        if (!event.isInterestedForInteractions ||
                          ...
                          final sections =
                              _buildSections(widget.reservablespot);
  ...
  List<PieChartSectionData> _buildSections(
      List<GetPopularReservableSpotModel> publicspot) {
...
class PositionedPieChartReservable extends StatelessWidget {
  const PositionedPieChartReservable({super.key, required this.publicspot});
  final List<GetPopularReservableSpotModel> publicspot;
